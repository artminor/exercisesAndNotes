...
function makeChain(text) {
	text = text.split(' ');
	const chain = {};
	for (let i = 0; i < text.length; i++) {
		let word = text[i];
		if (chain.hasOwnProperty(word)) {
			chain[word].push(text[i + 1]);
		} else chain[word] = [text[i + 1] + ' ' + text[i + 2]];
	}
	
	function makeLine(totalCount, chain) {
		let currentWord = getRandom(Object.getOwnPropertyNames(chain));
		let wordCount = 0;
		let line = currentWord;
		while (wordCount < totalCount) {
			let wordArr = chain[currentWord];
			if (wordArr.length === 0) {
				var nextWord = getRandom(Object.getOwnPropertyNames(chain));
			} else {
				nextWord = getRandom(chain[currentWord]);
			}
			line += ' ' + nextWord;
	...
	function getRandom(listOfWords) {
		let random = Math.floor(Math.random() * (listOfWords.length - 1));
		return listOfWords[random];
	}

	// remove puntuation (bonus part 2)
	//tested this part of function and it works, but won't run w/ .map when I call it with the words from text it gives me a .length error which I don't understand why???
	function normalize(word) {
		const punc = [',', '.', '?', '!', ';', ':', '"', "'"];
		for (let k = 0; k < word.length; k++) {
			let currentCharacter = word[k];
			for (let l = 0; l < punc.length; l++) {
				let p = punc[l];
	...
	// console.log(normalize("e!j'f!'"));

	//lines of poem
	var poem = '';
	for (var j = 0; j < 6; j++) {
		poem += makeLine(6, chain) + '\n';
	}

	return poem;
}

console.log(makeChain(text));
